...
class RBF_NN():
    def __init__(self, hidden_nodes_num, input_data_trainX, input_data_trainY, CRBF_Result):
        self.hidden_nodes_num = hidden_nodes_num  # 隐含层节点数
        self.input_data_trainX = input_data_trainX  # 训练样本的特征
        self.input_data_trainY = input_data_trainY  # 训练样本的标签
        self.CRBF_Result = CRBF_Result
    ...
    def fit(self):
        ...
        ## 隐含层到输出层权重和偏置
        W = tf.Variable(tf.random_normal([self.hidden_nodes_num, n_output]), name='W')
        b = tf.Variable(tf.random_normal([1, n_output]), name='b')

        ## RBF函数参数
        c = tf.Variable(tf.random_normal([self.hidden_nodes_num, n_input]), name='c')
        delta = tf.Variable(tf.random_normal([1, self.hidden_nodes_num]), name='delta')

        # 3.构造前向传播计算图
        ## 隐含层输出
        ### 特征样本与RBF均值的距离
        dist = tf.reduce_sum(tf.square(tf.subtract(tf.tile(X, [self.hidden_nodes_num, 1]), c)), 1)
...
if __name__ == '__main__':
    CRBF_Result = pd.read_csv('CRBF_Function')
    print('------------------------1.Load Data---------------------')
    trainX, trainY = load_data('data')
    print('------------------------2.parameter setting-------------')
    hidden_nodes_num = 20
    input_data_trainX = trainX
    input_data_trainY = trainY
    rbf = RBF_NN(hidden_nodes_num, input_data_trainX, input_data_trainY, CRBF_Result)
    rbf.fit()